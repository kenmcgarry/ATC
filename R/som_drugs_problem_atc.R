...
load("C:\\R-files\\ATC\\ATC_problem.RData")
...
train <- drugDATA[train_ind, ]  # we have "smp_size" amount of training data
test <-  drugDATA[-train_ind, ]   # we have whatever isnt in training data as test data (note minus sign)

som.atc <- som(as.matrix(train[,-1]),grid = somgrid(15, 15, "hexagonal"))
som.prediction <- predict(som.atc,   newdata = as.matrix(test[,-1]), trainX = as.matrix(train[,-1]), trainY = factor(train[,1]))
...
plot(som.atc, type="mapping", labels = train[,1], col = train[,1]+1, main = "ATC codes mapping plot")

## use hierarchical clustering to cluster the codebook vectors
som_cluster <- cutree(hclust(dist(som.atc$codes)), 10)
# plot these results:
...
som.xyf <- xyf(data = as.matrix(train[,-1]),Y = factor(train[,1]),grid = somgrid(15, 15, "hexagonal"))
par(mfrow = c(1, 2))
